...
function changeColor(e) {
    ...
    switch(currentMode) {
        case 'color' : this.style.backgroundColor = penColor;
                        break;
        case 'rainbow' : let h = () => Math.floor(Math.random() * 360); // Get random hue
                        let rainbow = `hsl(${h()}, 100%, 60%)`;
                        this.style.backgroundColor = rainbow;
                        break;
        case 'pastel' : let p = () => Math.floor(Math.random() * 360); // Get random hue
                        let pastel = `hsl(${p()}, 90%, 85%)`;
                        this.style.backgroundColor = pastel;
                        break;
        case 'eraser' : this.style.backgroundColor = 'transparent';
                        break;
    }
}
